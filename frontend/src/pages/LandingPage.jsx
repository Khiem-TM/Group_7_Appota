        ...
        <section className="relative overflow-hidden border-b border-outline-variant">
          ...
          <div className="relative mx-auto max-w-6xl px-4 pb-16 pt-14 sm:px-6 lg:px-8 lg:pt-20">
            <div className="mx-auto max-w-3xl text-center">
              <span className="inline-flex items-center rounded-full border border-outline-variant bg-surface-container-low px-3 py-1 text-xs font-medium tracking-[0.2em] text-primary-fixed-dim">
                ESPORTS READY
              </span>
              ...
              <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                ...
                <Link
                  to="/explore"
                  className="inline-flex items-center rounded-xl border border-outline-variant bg-surface-container-low px-5 py-3 text-sm font-medium text-white hover:border-outline-variant"
                >
                  Explore Tournaments
                </Link>
              </div>
            ...
            <div className="mt-12 rounded-2xl border border-outline-variant bg-surface-container-low/50 p-3 shadow-[0_0_0_1px_rgba(30,41,59,0.4)] sm:p-4">
              <div className="h-[300px] rounded-xl border border-outline-variant bg-landing-preview sm:h-[440px]" />
            </div>
          </div>
        </section>

        <section className="border-b border-outline-variant bg-surface/80">
          <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {formats.map((format) => {
                ...
                return (
                  ...
                  >
                    <Icon size={14} className="text-primary-fixed-dim" />
                    {format.label}
                  </div>
                );
              })}
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {featureCards.map((feature) => {
                ...
                return (
                  <article key={feature.title} className="rounded-2xl border border-outline-variant bg-surface-container-low p-5">
          ...
          <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
            <div className="mb-6 flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-on-surface-variant">Join active events happening right now.</p>
              </div>
              <Link to="/explore" className="text-sm text-primary-fixed-dim hover:text-primary-fixed">
                View All
              </Link>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {trendingTournaments.map((tournament) => (
                <article key={tournament.id} className="rounded-2xl border border-outline-variant bg-surface-container-low p-4">
                  <div className="h-28 rounded-xl border border-outline-variant bg-landing-card-preview" />
                  ...
                  <div className="mt-4 flex items-center justify-between text-sm text-on-surface-variant">
                    ...
                    <Link to="/explore" className="text-primary-fixed-dim hover:text-primary-fixed">
                      {tournament.cta}
                    </Link>
                  </div>
                </article>
      ...
      <footer className="border-t border-outline-variant bg-surface">
        <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-10 sm:px-6 lg:flex-row lg:justify-between lg:px-8">
          <div>
            ...
            <p className="mt-2 max-w-sm text-sm text-on-surface-variant">The modern operating system for esports organizers.</p>
          </div>

          <div className="grid grid-cols-2 gap-8 text-sm text-on-surface-variant sm:grid-cols-3">
            <div>
